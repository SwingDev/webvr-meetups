...
// @source: https://github.com/facebook/react-vr/blob/4b5dd8df5df0a5223c94f3486e8c40ee2423a120/OVRUI/src/Control/VREffect.js
...
export default class VREffect {
  ...
  requestPresent () {
    if (!this.vrDisplay) {
      return Promise.reject(new Error('No VR Display!'))
    }
    ...
    return this.vrDisplay
      ...
      .then(() => this._configureRendererForVRDisplay())
  }
  ...
  render (scene, camera, frameData) {
    if (this.vrDisplay && this.vrDisplay.isPresenting) {
      ...
      // Only allow stereo background rendering if both backgrounds have been
      // set otherwise the user will see the background in only one eye.
      const isStereoBackgroundReady = (
        !!scene.backgroundLeft && !!scene.backgroundRight
      )
